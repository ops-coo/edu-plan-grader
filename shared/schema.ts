...
export const dataSourceConfigs = sqliteTable("data_source_configs", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  type: text("type").notNull(), // "bigquery" | "google_docs" | "google_sheets" | "google_drive" | "github" | "local_file"
  name: text("name").notNull(),
...
export const normalizedDataRecords = sqliteTable("normalized_data_records", {
  ...
  contentType: text("content_type").notNull(), // "business_plan" | "financial_data" | "brainlift" | "project_status" | "milestone" | "pnl" | "other"
