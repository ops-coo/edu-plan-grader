...
export function registerPipelineRoutes(app: Express) {
  ...
  app.get("/api/data-sources/:id", (req, res) => {
    const source = storage.getDataSourceConfig(parseInt(req.params.id));
  ...
  app.patch("/api/data-sources/:id", (req, res) => {
  ...
  app.post("/api/data-sources/:id/discover", async (req, res) => {
    const source = storage.getDataSourceConfig(parseInt(req.params.id));
    if (!source) return res.status(404).json({ error: "Data source not found" });
  ...
  app.get("/api/data-records/:id", (req, res) => {
    const record = storage.getNormalizedDataRecord(parseInt(req.params.id));
  ...
  app.get("/api/business-units/:id/data-records", (req, res) => {
    const records = storage.getDataRecordsForBusinessUnit(parseInt(req.params.id));
    res.json(records);
  });

  app.post("/api/data-records/:id/link-bu", (req, res) => {
    const dataRecordId = parseInt(req.params.id);
  ...
  app.delete("/api/data-records/:id/link-bu/:buId", (req, res) => {
    const deleted = storage.deleteDataRecordBuLink(
      parseInt(req.params.id),
      parseInt(req.params.buId),
    );
