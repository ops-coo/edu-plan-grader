...
export function registerPipelineRoutes(app: Express) {
  ...
  function omitCredentials({ credentialsEnc, ...rest }: any) {
    return rest;
  }

  app.get("/api/data-sources", (_req, res) => {
    const sources = storage.getDataSourceConfigs().map(omitCredentials);
    res.json(sources);
  });
  ...
  app.get("/api/data-sources/:id", (req, res) => {
    ...
    res.json(omitCredentials(source));
  });

  app.post("/api/data-sources", pipelineRateLimit, (req, res) => {
    const { type, name, authMethod, pollIntervalMin, enabled, config } = req.body;
    if (!type || !name) {
      return res.status(400).json({ error: "type and name are required" });
    }
    const validTypes = getAvailableConnectorTypes();
    if (!validTypes.includes(type)) {
      return res.status(400).json({ error: `Invalid connector type: ${type}. Available: ${validTypes.join(", ")}` });
    }
    ...
    res.status(201).json(omitCredentials(source));
  });
  ...
  app.patch("/api/data-sources/:id", pipelineRateLimit, (req, res) => {
    ...
    if (req.body.type !== undefined) {
      if (!getAvailableConnectorTypes().includes(req.body.type)) {
    ...
    if (req.body.config !== undefined) updates.config = req.body.config ? JSON.stringify(req.body.config) : null;

    const updated = storage.updateDataSourceConfig(id, updates);
    if (!updated) return res.status(409).json({ error: "Data source could not be updated" });
    res.json(omitCredentials(updated));
  });
    ...
    try {
      ...
      for (const result of results) {
        ...
        const event = storage.createIngestionEvent({
          ...
          processingMs: Date.now() - itemStart,
        });
        ...
        if (!unchanged && contextualize) {
          try {
            ...
            const record = storage.runInTransaction(() => {
              storage.supersedeLatestRecord(source.id, result.externalId);
              const rec = storage.createNormalizedDataRecord({
