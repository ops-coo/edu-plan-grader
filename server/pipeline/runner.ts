...
export class PipelineRunner {
  ...
  async runCycle(): Promise<void> {
    ...
    try {
      ...
      for (const source of sources) {
        ...
        const now = Date.now();
        const parsedPoll = config.lastPolledAt ? new Date(config.lastPolledAt).getTime() : 0;
        const lastPolled = Number.isFinite(parsedPoll) ? parsedPoll : 0;
  ...
  private async processSource(source: any, config: ConnectorConfig): Promise<void> {
    ...
    console.log(`[pipeline] ${source.name}: ${changedItems.length} items to check`);
