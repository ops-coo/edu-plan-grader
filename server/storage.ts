...
export class DatabaseStorage implements IStorage {
  ...
  getDataRecordsForBusinessUnit(buId: number): NormalizedDataRecord[] {
    const links = db.select().from(dataRecordBuLinks)
      .where(eq(dataRecordBuLinks.businessUnitId, buId))
      .all();
    if (links.length === 0) return [];
    const recordIds = links.map(l => l.dataRecordId);
    return db.select().from(normalizedDataRecords)
      .all()
      .filter(r => recordIds.includes(r.id) && r.isLatest === 1);
  }
  ...
  deleteDataRecordBuLink(dataRecordId: number, buId: number): boolean {
    const all = db.select().from(dataRecordBuLinks)
      .where(eq(dataRecordBuLinks.dataRecordId, dataRecordId))
      .all()
      .filter(l => l.businessUnitId === buId);
    if (all.length === 0) return false;
    db.delete(dataRecordBuLinks).where(eq(dataRecordBuLinks.id, all[0].id)).run();
    return true;
  }
