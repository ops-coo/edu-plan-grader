...
sqlite.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_ingestion_events_lookup ON ingestion_events (source_config_id, external_id, fetched_at);
  CREATE INDEX IF NOT EXISTS idx_normalized_records_lookup ON normalized_data_records (source_config_id, external_id, is_latest);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_normalized_records_latest_unique ON normalized_data_records (source_config_id, external_id) WHERE is_latest = 1;
  CREATE INDEX IF NOT EXISTS idx_data_record_bu_links_bu ON data_record_bu_links (business_unit_id);
`);
...
export class DatabaseStorage implements IStorage {
  ...
  getIngestionEvents(sourceConfigId?: number, limit: number = 100): IngestionEvent[] {
    if (sourceConfigId) {
      return db.select().from(ingestionEvents)
        ...
        .orderBy(desc(ingestionEvents.fetchedAt), desc(ingestionEvents.id))
        .limit(limit)
        .all();
    }
    return db.select().from(ingestionEvents)
  ...
  getNormalizedDataRecords(filters: DataRecordFilters): NormalizedDataRecord[] {
    ...
    return query
      .orderBy(desc(normalizedDataRecords.createdAt), desc(normalizedDataRecords.id))
  ...
  getRecentChanges(limit: number = 50): ChangeTracking[] {
    return db.select().from(changeTracking)
      .orderBy(desc(changeTracking.detectedAt), desc(changeTracking.id))
      .limit(limit)
      .all();
  }
